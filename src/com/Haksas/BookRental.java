...
//		BookList(query);
		...
		btnReturn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				if(Pivot.equals("Y"))
					JOptionPane.showMessageDialog(null, "빌려가지 않은 책입니다.", "Message", JOptionPane.ERROR_MESSAGE);
				else {
					int result = JOptionPane.showConfirmDialog(null, "반납하시겠습니까?", "Message", JOptionPane.YES_NO_OPTION);
					
					switch (result) {
					case JOptionPane.OK_OPTION:
						JOptionPane.showMessageDialog(null, "반납완료", "Message", JOptionPane.OK_OPTION);
	...
	private void query(RentalStatus status, int index, String inValue) {
		...
		case LOAN:
			...
			try {
				stmt = conn.createStatement();
				if(inValue == null)
					stmt.executeUpdate("insert into BookRental(RentStudent_id, RentBook_No, BookRental_Date, BookRental_loan) values('"+ tf_hakbun.getText() +"','" + tf_bookNo.getText() + "','" + time + "', '사용중')");
